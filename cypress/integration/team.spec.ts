...
context('Team Integration Test', () => {
    ...
    it('Tests National Admin can deactivate an user', () => {
      ...
      cy.contains('Sulaka DNRPC District Office').click()
    ...
    cy.get('#firstNamesEng').type('Nafis')
  ...
  it('Tests Local admin can edit an user', () => {
    ...
    cy.createPin()
    cy.get('#navigation_team').click()
    cy.get('#user-item-0-menuToggleButton').click()
    cy.get('#user-item-0-menuItem0').click()
    cy.get('#btn_change_firstNamesEng').click()
    cy.get('#firstNamesEng').type(' antor')
